...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[use optics]
        ...
        \path[name path=lensnf] (reference.lens north) -- (reference.lens north -| sample.north);
        \path[name path=lenssf] (reference.lens south) -- (reference.lens south -| sample.south);
        \path[name intersections={of=lensnf and sphere,by=sphere north}];
        \path[name intersections={of=lenssf and sphere,by=sphere south}];

        \draw[name path=expnsamplen] (beam expander.north east) |- (reference.lens north) -- (sphere south) -- (reference.lens south -| sample.south);
        \draw[name path=expssamples] (beam expander.south east) |- (reference.lens south) -- (sphere north) -- (reference.lens north -| sample.north);
    \end{tikzpicture}
    \caption{120mm converger reference setup collimated}
    \label{fig:curv_setup_d}
\end{figure}
