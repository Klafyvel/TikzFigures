...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[use optics]
        ...
        \draw (reference.north) to[dim arrow={label=120mm}]  (sample.mirror center |- reference.north);

        % rays
        \draw[->-] (laser.aperture east) -- (sample.arc center);
        \draw[red, ->>-] (sample.mirror center) -| (camera.aperture west);
        \draw[name path=expnsamplen] (beam expander.north east) |- (reference.lens north) -- (sample.mirror center);
        \draw[name path=expssamples] (beam expander.south east) |- (reference.lens south) -- (sample.mirror center);
    \end{tikzpicture}
    \caption{120mm converger reference setup focused on center of curvature}
    \label{fig:curv_setup_b}
\end{figure}
